...
#   end
  ...
  def add_member(entity)
    if entity.respond_to?(:user_id) && entity.respond_to?(:id)
      # Assume it's a Participant (AssignmentParticipant or CourseParticipant)
      return { success: false, error: "Participant is already on the team." } if participants.exists?(id: entity.id)
      return { success: false, error: "Team is full." } if full?
      ...
      # Assume it's a plain User (used for team_membership)
      return false if full? || users.include?(entity)
      return false unless validate_membership(entity)
      team_members.create(user: entity, role: 'member')

    else
      raise ArgumentError, "Unsupported type for add_member: #{entity.class}"
    end
  rescue StandardError => e
    { success: false, error: e.message }
  end
